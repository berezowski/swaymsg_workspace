...
pub fn setup_4_workspaces_across_3_outputs() -> (Rc<Workspaces>, Rc<RefCell<Vec<String>>>) {
	...
	let connection: RefCell<Box<dyn ConnectionProxy>> =
		RefCell::new(Box::new(MockConnection { commandhistory: commands.clone() }));
	let ipcadapter = MockIPCAdapter { connection,
	                                  ...
	                                  outputs: vec![
	                                                Box::new(MockOutput { focused: true,
	                                                                      name:
		                                                                      "eDP-1".to_string(), }),
	                                                Box::new(MockOutput { focused: false,
	                                                                      name:
		                                                                      "HDMI-1".to_string(), }),
	                                                Box::new(MockOutput { focused: false,
	                                                                      name:
